...
### 1. 灵活的参数组合
支持四种使用模式，适应不同的信息掌握程度：
- **完全自动提取**：不提供license参数，工具自动扫描并提取所有license信息
- **指定license名称**：提供 `-l` 参数，工具查找与该名称匹配的license文件
- **指定license路径**：提供 `-p` 参数，工具从指定路径提取license名称
- **同时指定名称和路径（推荐）**：同时提供 `-l` 和 `-p` 参数，跳过自动检测，结果最准确

### 2. 智能的误匹配过滤
工具内置两层保护机制，有效过滤误报：

**第一层：路径有效性检查**
- 只接受项目根目录下的license文件（如 `LICENSE`, `LICENSE.MIT`）
- 只接受根目录下LICENSES目录中的文件（如 `LICENSES/MIT.txt`）
- 不接受其他目录下的文件，避免误识别代码文件中的license声明

**第二层：匹配质量检查**
- 检查是否为 `.LICENSE` 规则匹配（表示完整的license文本匹配）
- 如果非 `.LICENSE` 规则匹配，且匹配长度小于20字符，则判定为误匹配并过滤

### 3. 参数一致性校验
当同时指定license名称和路径时，工具会进行一致性校验：
- 验证指定的license文件路径是否存在
- 验证文件内容与指定的license名称是否匹配
- 如果不匹配，输出警告提示用户人工核对

### 4. Copyright信息去重
自动去除重复的copyright声明，只保留唯一的copyright记录。同时过滤掉文档文件（`.md`, `.rst`, `.txt`等）中的copyright信息，避免重复。

### 5. 嵌套目录自动检测
自动检测并处理源码包解压后的嵌套目录结构（如 `package.zip-extract/project-name/...`），正确识别项目根目录。

### 6. 完善的日志系统
支持三级日志输出：
- **默认模式**：显示 INFO, OK, WARN, ERROR 级别日志
- **调试模式**（`LOG_LEVEL=DEBUG`）：显示全部日志，包括DEBUG调试信息
- **静默模式**（`LOG_LEVEL=QUIET`）：只显示ERROR错误信息
