...
LICENSE_FILE_PATTERNS = [
    ...
    r"^notice[-_.].*",      # NOTICE.txt, NOTICE.md
...
LICENSE_EXTENSIONS = {
    ...
    ".lesser", ".gpl", ".GPL", ".apache", ".APACHE",
    ".mit", ".MIT", ".bsd", ".BSD", ".lgpl", ".LGPL",
    ".mpl", ".MPL", ".ofl", ".OFL", ".unlicense",
    ".artistic", ".cc0", ".CC0",
}
...
def extract_licenses(
    ...
    """
    从scancode结果中提取license信息。
    
    仅提取项目根目录下包含"license"、"copying"等关键字的文件，
    以及根目录下包含"license"字样目录中的文件的license信息。
    
    Args:
        result_json: scancode输出的JSON文件路径
        output_txt: 输出文件路径
        root_path: 项目根目录路径，如果为None则从JSON中推断
    ...
    if root_path is None:
        warn("Cannot determine root path, using all license files found")
    
    # 确定要检查的license文件集合（相对路径）
    target_license_files = set()
    
    if root_path:
        target_license_files = get_license_files_from_root(root_path)
        info(f"Root path: {root_path}")
        info(f"Found {len(target_license_files)} potential license files in root directory")
        for f in sorted(target_license_files):
            info(f"  - {f}")
    
    # 构建目标license文件的文件名集合（用于更宽松的匹配）
    ...
    debug(f"Target license filenames: {target_license_filenames}")
    
    # 收集license信息
    license_records = []
    seen_licenses = set()  # 用于去重
    
    for file_info in data.get("files", []):
        file_path = file_info.get("path", "")
        file_name = Path(file_path).name.lower()
        
        # 检查该文件是否在目标license文件集合中
        is_target_file = False
        
        if target_license_files:
            # 检查相对路径是否匹配
            if file_path in target_license_files:
                is_target_file = True
            # 检查文件名是否在目标文件名集合中
            elif file_name in target_license_filenames:
                is_target_file = True
            else:
                # 检查路径中是否包含LICENSES目录
                parts = Path(file_path).parts
                for part in parts:
                    if is_license_directory(part):
                        is_target_file = True
                        break
        else:
            # 如果无法确定target_license_files，对所有文件检查
            if is_license_file(Path(file_path).name):
                is_target_file = True
        
        if not is_target_file:
            continue
        
        # 提取license信息 - 使用scancode新版本的字段格式
        ...
        # 构建唯一标识用于去重
        unique_key = (detected_spdx, detected_license)
        if unique_key in seen_licenses:
            continue
        seen_licenses.add(unique_key)
    ...
    with output_txt.open("w", encoding="utf-8") as f:
        ...
        if not license_records:
            f.write("No license information found in license-related files.\n")
        else:
            ...
            for i, record in enumerate(license_records, 1):
                ...
                f.write(f"SPDX Identifier: {record['spdx_identifier']}\n")
                f.write(f"License Expression: {record['license_expression']}\n")
                if record['matched_text']:
                    # 限制matched_text的长度
